package org.firstinspires.ftc.teamcode.util.tensorflowapi.ImageClassification;
...
import org.firstinspires.ftc.teamcode.util.tensorflowapi.TensorProcessingException;
...
public class TensorImageClassifier {
    ...
    private static MappedByteBuffer loadModelFile(AssetManager assets, String modelFilename)
            throws IOException {
        File file = new File(Environment.getExternalStorageDirectory() + "/FIRST/tflitemodels/" + modelFilename);

        FileInputStream fis = new FileInputStream(file);

        return fis.getChannel().map(FileChannel.MapMode.READ_ONLY, 0L, file.length());
